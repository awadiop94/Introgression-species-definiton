...
classification = {}
ANI = {}
genome_csv = species_dir / "ANI_results/genome_clusters.csv"
...
def process_outgroup(outgroup: str):
    ...
    # Verification output
    verification_file = og_dir / f"verification_{1 if outgroup=='outgroup1' else 2}.txt"
    with verification_file.open("w") as h:
        for file_key in names:
            ...
            for spi in storage:
#                print(f"{file_key=} {spi=} {storage[spi]=} {tmp[spi]=} {number[spi]=}")
                box = storage[spi]
                strain_tank, tank = [], []
                for short in box:
                    st = real_name[file_key][short]
