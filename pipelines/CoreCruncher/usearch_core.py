					...
					for prot2 in liste[sp]:
						...
						if 1==1:
							if resu1 in bbh[sp] and  resu2 in bbh[sp] :
								...
								print(sp,'/',nb)
								os.system('/overflow/bobaylab/adiop2/usearch61  -usearch_global  '   + PATH + prot2 + ' -db  ' + PATH  + prot1  + option +  ' -id ' + str(score) + ' -maxaccepts 3  -blast6out ' + out_PATH + 'CC/' + prot2 + '-' + prot1 )
							elif resu2 in bbh[sp]:
								os.system('/overflow/bobaylab/adiop2/usearch61  -usearch_global  '  + PATH + prot1 + ' -db  ' + PATH  + prot2  + option + ' -id ' + str(score) + ' -maxaccepts 3  -blast6out ' + out_PATH + 'CC/' + prot1 + '-' + prot2 )
							else:
								os.system('/overflow/bobaylab/adiop2/usearch61  -usearch_global  '  + PATH  + prot1 + ' -db  ' + PATH   + prot2  + option + ' -id ' + str(score) + ' -maxaccepts 3  -blast6out ' + out_PATH + 'CC/' + prot1 + '-' + prot2 )
								os.system('/overflow/bobaylab/adiop2/usearch61  -usearch_global  '  + PATH + prot2 + ' -db  ' + PATH  + prot1 +  option + ' -id ' + str(score) + ' -maxaccepts 3  -blast6out ' + out_PATH + 'CC/' + prot2 + '-' + prot1 )
