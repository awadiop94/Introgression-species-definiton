...
tmp = os.listdir("../root1/")

forest=[]
for file in tmp:
	if file.endswith(".tree") and file.startswith("renamed"):
		forest.append(file)

print (forest)

for file in forest:
	sp = file.split(".tree")[0]
	unique=[]
	print (file)
	new_file = file.split(".tree")[0] + ".txt"
	h=open("../root1/" + new_file,"w")
	f=open("../root1/" +  file ,"r")
	l=f.readline()
	l = l.strip("\n")
	l = l.strip(";")
	...
	h=open("../root1/" + "dichotomies_" + out_name + ".txt","w")
	for st in dico:
		#print st," ",dico[st]
	...
	h=open("../root1/" + "roots_" + out_name + ".txt","w")
	for root in ROOTS:
